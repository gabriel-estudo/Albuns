<!DOCTYPE html>
<html lang="pt-BR">

<head>
    <meta charset="UTF-8" />
    <meta name="viewport" content="width=device-width, initial-scale=1.0" />
    <link rel="icon" type="image/png" href="img/favIcon.png">
    <link rel="stylesheet" href="css/menu.css" />
    <link rel="stylesheet" href="css/shows.css" />
    <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/7.0.1/css/all.min.css" />
    <script src="js/shows.js"></script>
    <title>Shows</title>
</head>

<body>
    <?php include_once 'menu.php'; ?>

    <div class="container-melhor">
        <div class="melhor">
            <div class="melhor-titulo">Show</div>
            <div class="container-formulario">
                <form action="" id="formularioShows" enctype="multipart/form-data">
                    <input type="hidden" id="ctrlFormulario" value="incluir">
                    <input type="hidden" id="frmId" name="frmId" value="0">
                    <div>
                        <button type="button" class="botao-confirmar" id="botaoConfirmarInclusao">Confirmar</button>
                    </div>
                    <div class="container-campos">
                        <div>
                            <label for="">Banda:</label>
                            <select class="select-banda-id" name="frmBandaId" id="frmBandaId"></select>
                        </div>
                        <div>
                            <label for="">Local:</label>
                            <input class="input-local" type="text" id="frmLocal" name="frmLocal">
                        </div>
                        <div>
                            <label for="">Data:</label>
                            <input class="input-data" type="date" value="2025-10-20" id="frmData" name="frmData">
                        </div>
                    </div>
                </form>
            </div>
        </div>
        <div class="melhor">
            <div class="melhor-titulo">Grade de registros</div>
            <div class="container-tabela">
                <table class="tabela">
                    <thead>
                        <tr>
                            <th class="tabela-titulo" style="width: 20%">Banda</th>
                            <th class="tabela-titulo" style="width: 35%">Local</th>
                            <th class="tabela-titulo" style="width: 25%">Data</th>
                            <th class="tabela-titulo" style="width: 10%;">Opções</th>
                        </tr>
                    </thead>
                    <tbody id="gradeRegistros">
                    </tbody>
                </table>
            </div>
        </div>
    </div>
</body>

</html>